...
/* Country dialling codes for the SMS field, in Brevo's own order. "<code> <ISO>" */
const BREVO_SMS_COUNTRIES = ('93 AF,358 AX,355 AL,213 DZ,1684 AS,376 AD,244 AO,1264 AI,672 AQ,1268 AG,54 AR,374 AM,'
...
/* Default dialling country. Brevo's main.js reads the <select>'s current value for the
   calling code, so the matching option has to carry `selected` or it falls back to the
   first entry in the list (+93 AF). */
const BREVO_SMS_DEFAULT_ISO = 'FR';

function brevoSmsField(v) {
  const options = BREVO_SMS_COUNTRIES.map(function (c) {
    const parts = c.split(' ');
    const sel = parts[1] === BREVO_SMS_DEFAULT_ISO ? ' selected' : '';
  ...
  return ''
    ...
    + '<label class="entry__label" style="font-weight: 700; text-align: left; font-family:Helvetica, sans-serif;'
    + ' font-size:16px; font-weight:700; text-align:left; color:' + v.labelColor + ';" for="SMS"'
    ...
    + '</div>'
    + '<div class="sib-sms-tooltip"><div class="sib-sms-tooltip__box">The SMS field must contain between 6 and 19'
    + ' digits and include the country code without using +/0 (e.g. 1xxxxxxxxxx for the United States)</div>'
...
function brevoFormHTML(variant) {
  ...
  return ''
    ...
    + ' form="sib-form" type="submit">Submit</button>'
    + '</div></div>'
